...
#nnnnnnnnnnnnnnnnnnnnnnnnnnn
  ...
  observeEvent(input$map_shape_click, { 
    ...
    if (!is.null(selected_suburb)){
      ...
      ####### existing business
      #HTML({paste('<p style="color:#3A479B;  font-size: 15px;">',paste('Number of Hairdressing Business: <strong>',no),'</strong></p>')})
      output$existing_business <- renderText({
        HTML({paste('<p style="color:#3A479B;  font-size: 15px;"> Number of Hairdressing Business: <strong>',get_business_count(selected_suburb),'</strong></p>')})
      })
