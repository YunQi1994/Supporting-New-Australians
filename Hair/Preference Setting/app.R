...
###ranking data, the variable name is weird but they are not dummy

dummy <- read.csv('data/ranking_hair.csv', stringsAsFactors = F)
    ...
    fluidRow(
      column(7,
             ...
             mainPanel(style = 'color: #3A479B; font-size: 2rem;  font-family: Montserrat;left: 23%; top: 3rem',
                       ...
                       a(style = 'margin-right: -20%; font-size: 2rem; margin-left: 5%; color: #3A479B' ,"Click here to explore!", href="http://www.demeter.gq/Home/Hairdressing", target="_top")
             )
      ...
      column(5,
             wellPanel(
               ...
               div(div(style = 'display: inline-block;' ,htmlOutput('title2')),
...
server <- function(input, output, session){
  ...
  output$title1 <-renderText({ 
    help_text = HTML('<p style="color:#16224D;  font-weight:bold; font-size: 20px;">Please Choose Your Preferences</p>')
    #help_text = 'Please hover on suburb to see more infomation'
  })
  output$title2 <-renderText({ 
    help_text = HTML('<p style="color:#16224D;  font-weight:bold; font-size: 20px;">Best Suburbs Based On Your Preferences</p>')
    #help_text = 'Please hover on suburb to see more infomation'
  })
  ...
  observe({
    ...
    output$business01 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Hairdressing Business: ',get_business_count(suburb01),'</p>')})
    })
    ...
    output$business02 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Hairdressing Business: ',get_business_count(suburb02),'</p>')})
    })
    ...
    output$business03 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Hairdressing Business: ',get_business_count(suburb03),'</p>')})
    })
