...
server = function(input, output, session){
  ...
  observeEvent(input$map_shape_click, { 
    ...
    if (!is.null(selected_suburb)){
      ...
      data <- search(cityID, api_key = key2, query = selected_suburb, cuisine = cuisine_to_search)
      ...
      if (!typeof(data) == 'logical'){
        ...
        if (length(data[[1]] >0)){
          ...
          existing_business = HTML({paste('<p style="color:#3A479B;  font-size: 15px;">',paste('Number of Turkish Restaurants: <strong>',no),'</strong></p>')})
          ...
          existing_business = HTML('<p style="color:#3A479B; font-size: 15px;">Number of Turkish Restaurants: <strong>0</strong></p>')
          output$existing_business <- renderText({ 
            existing_business
          })
          output$restaurant_text <- renderText({
