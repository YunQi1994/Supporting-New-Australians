...
get_center <- function(suburbs){
  ...
  print(rbind(center_1,center_2,center_3))
...
dummy <- read.csv('data/ranking_lb.csv', stringsAsFactors = F)
...
server <- function(input, output, session){
  ...
  output$title2 <-renderText({ 
    help_text = HTML('<p style="color:#16224D;  font-weight:bold; font-size: 20px;">Best Suburbs Based On Your Choice</p>')
    #help_text = 'Please hover on suburb to see more infomation'
  })
  ...
  observe({
    ...
    output$business01 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Lebanese Restaurants: ',get_business_count(suburb01),'</p>')})
    })
    ...
    output$business02 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Lebanese Restaurants: ',get_business_count(suburb02),'</p>')})
    })
    ...
    output$business03 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Lebanese Restaurants: ',get_business_count(suburb03),'</p>')})
    })
