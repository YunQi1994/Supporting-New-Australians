...
cuisine_to_search <- cuisine_reference[["LB"]]
...
ui = fluidPage(
  ...
  mainPanel(style = "background: #F0F4FF; width: 100%; height: 100%",
            ...
            a(style = 'font-size: 1.5rem; color: #3A479B' ,"Comparing suburbs? Click Here", href="http://www.demeter.gq/Home/CompareLB", target="_top")
            ), # return button
            ...
            column(4, offset = 0, 
                   wellPanel(style = "background: white; margin-left: -5%;margin-right:-60% ;height:220px; width: 100%",
                             ...
                             mainPanel(id = 'info_box', style="text-align: left; width: 100%",
                                       tags$div(div(style='display: inline-block; color:#D4AF37; font-size:2rem; font-weight:bold', id = 'highlevel1', htmlOutput(outputId = 'suburb_box')),
                   ...
                   wellPanel(style = "margin-left: -5%;background: white; height: 372px;width: 100%; margin-top: -3%",
                             ...
                             tags$br(),
                             div(id = 'radio button',radioGroupButtons("radio_subset", label = HTML({paste('<p style="color:#D4AF37; margin-top:-5px;margin-bottom: 10%;margin-left: 10%;margin-right: -10%; font-size:18px"><strong>To See Suburbs That Have:</strong></p>')}), # link to output$radio_subet in the server 
...
server = function(input, output, session){
  ...
  observeEvent(input$reset_button, {
    ...
    # new updated
    output$restaurant_text <- renderText('')
  ...
  ### a function to get avg_cost
  get_avg_cost <- function(subset_df){
    avg_cost <- ''
    if (length(subset_df[[1]]) > 0){
      print('>0')
      no <- length(subset_df[[1]])
      avg_cost <- paste('Average Spending For Two:<strong>',round(sum(subset_df$average_cost_for_two)/no),'$<strong>')
    }
    return(avg_cost)
  }
  ...
  observeEvent(input$map_shape_click, { 
    ...
    if (!is.null(selected_suburb)){
      ...
      print(paste('school df:',school_to_show,'end'))
      ...
      print(paste('childcare df:',childcare_to_show,'end'))
      ...
      output$map <- renderLeaflet({
        ...
        hideGroup(group = control_group[5:6]) %>%
          ...
          addPolygons(data = vic,
                      weight = 0,
                      stroke = 0,
                      fillColor = ~mypal_lb(vic$LB), # heatmap colour 
                      fillOpacity = 0,
                      label = ~suburb,
                      labelOptions = labelOptions(
                        opacity = 0),
      ...
      #######start of markers updated############
      
      school_to_show <- subset(school, Address_Town == selected_suburb)
      childcare_to_show <- subset(childcare, Suburb == selected_suburb)
      legal_to_show <- subset(legal, Suburb == selected_suburb)
      
      ### when a suburb has no school
      if (length(school_to_show$Address_Town) == 0){
        ### when only have childcare 
        if (length(childcare_to_show$Suburb) > 0 && length(legal_to_show$Suburb) == 0){
          
          # change the map view - zoom in and then add schools
          leafletProxy('map') %>%
            # plot childcare markers
            addAwesomeMarkers(data = childcare_to_show,
                              lng = childcare_to_show$Longitude,
                              lat = childcare_to_show$Latitude,
                              icon = awesomeIcons(
                                icon = "child",
                                library = "fa",
                                markerColor = "green"),
                              popup = paste('Name:',childcare_to_show$Name.of.Business,'<br>',
                                            'Type:',childcare_to_show$Business.Type,'<br>',
                                            'Address:',childcare_to_show$Address,'<br>',
                                            'Phone:',childcare_to_show$Phone.Number),
                              popupOptions =  popupOptions(noHide = F,
                                                           direction = "center",
                                                           style = list(
                                                             "color" = "black",
        ...
        ### when a suburb has only legal
        if (length(legal_to_show$Suburb) > 0 && length(childcare_to_show$Suburb) == 0){
          
          # change the map view - zoom in and then add schools
          leafletProxy('map') %>%
            # plot legal markers
            addAwesomeMarkers(data = legal_to_show,
                              lng = legal_to_show$Longitude,
                              lat = legal_to_show$Latitude,
                              icon = awesomeIcons(
                                icon = "gavel",
                                library = "fa",
                                markerColor = "purple"),
                              popup = paste('Name:',legal_to_show$Name.of.Business,'<br>',
                                            'Type:',legal_to_show$Business.Type,'<br>',
                                            'Address:',legal_to_show$Address,'<br>',
                                            'Phone:',legal_to_show$Phone.Number),
                              popupOptions =  popupOptions(noHide = F,
                                                           direction = "center",
                                                           style = list(
                                                             "color" = "black",
      ...
      ## when a suburb only has school
      if (length(legal_to_show$Suburb) == 0 && length(childcare_to_show$Suburb) == 0 && length(school_to_show$Address_Town) > 0){
        leafletProxy('map') %>%
          # plot school markers
          addAwesomeMarkers(data = school_to_show,
                            lng = school_to_show$X,
                            lat = school_to_show$Y,
                            icon = awesomeIcons( # use awesome icons. can look up icons online to 
                              icon = "graduation-cap",
                              library = "fa",
                              markerColor = "lightred"), 
                            popup = paste('Name:',school_to_show$School_Name,'<br>',
                                          'Type:',school_to_show$School_Type,'<br>',
                                          'Address:',school_to_show$Address_Line,',',school_to_show$Address_Town,',',school_to_show$Address_State,',',school_to_show$Adress_Postcode),
                            popupOptions =  popupOptions(noHide = F,
                                                         direction = "center",
                                                         style = list(
                                                           "color" = "black",
      ...
      ## if a suburb only have school and childcare
      if (length(legal_to_show$Suburb) == 0 && length(childcare_to_show$Suburb) > 0 && length(school_to_show$Address_Town) > 0){
        leafletProxy('map') %>%
          # plot childcare markers
          addAwesomeMarkers(data = childcare_to_show,
      ...
      if (selected_suburb %in% school$Address_Town && length(transport_to_show$suburb) > 0) {
        ...
        train_st <- factor(train_to_show[[3]])
        print(levels(train_st))
        # no. of train stations
        output$train_stations_count <- renderText({
          
          length(levels(train_st))
        })
        print(factor(train_to_show[[3]]))
        # tram #
      ...
      # 
      cityID <- get_city_ID(selected_suburb)
      data <- search(cityID, api_key = key3, query = selected_suburb, cuisine = cuisine_to_search)
      
      # plot only if there is result new updated
      if (!typeof(data) == 'logical'){
        #is_found = TRUE
        ...
        print(typeof(data))
        if (length(data[[1]] >0)){
          if (str_detect(selected_suburb,'-')){
            
            ss <- trimws(str_split(selected_suburb,'-',simplify = T))
            
            tmp1 <- subset(data, str_detect(data$locality,ss[1]))
            tmp2 <- subset(data, str_detect(data$locality,ss[2]))
            data <- rbind(tmp1,tmp2)
            print(paste(selected_suburb,tmp1))
            print(data$locality)
          }
          else {
            data <- subset(data, str_detect(data$locality,selected_suburb))
            print('not combined')
            print(selected_suburb)
          }
          
          no <- no_restaurants(data)
          
          
          print ('restaurants found')
          #data['avg_cost'] = paste('AVG cost for two:', data$average_cost_for_two,'$')
          #data['d_price_range'] = paste('Price range (1-5):', data$price_range)
          #pal <- colorFactor(c("navy", "#DC143C",'black'), domain = c("MDE", "LB","TK"))

          
          existing_business = HTML({paste('<p style="color:#3A479B;  font-size: 15px;">',paste('Number of Lebanese Restaurants: <strong>',no),'</strong></p>')})
          ...
          output$restaurant_text <- renderText({
            restaurant_info <- HTML({paste('<p style="color:#3A479B;; font-size: 15px;">',get_avg_cost(data),'</strong></p>')})
          })
          ...
          existing_business = HTML('<p style="color:#3A479B; font-size: 15px;">Number of Lebanese Restaurants: <strong>0</strong></p>')
          output$existing_business <- renderText({ 
            existing_business
          })
          output$restaurant_text <- renderText({
      ...
      if (typeof(data) == 'logical'){
        # when api key exceeds the limited uasage. 
        existing_business = HTML('<p style="color:#3A479B; font-size: 15px;">Number of Lebanese Restaurants: <strong>0</strong></p>')
        print ('restaurants not found; API Key Exceeds limits!!!')
        output$existing_business <- renderText({ 
          existing_business
        })
      }
