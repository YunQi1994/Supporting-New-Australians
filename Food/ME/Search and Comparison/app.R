...
cuisine_to_search <- cuisine_reference[["ME"]]
...
get_business_count <- function(suburb){
  ...
  if (length(business_subset) > 0){
    no = business_subset[[3]] ###### require update when changing map type
  }
  return(no)
}
                                      ...
                                      div(id = 'comparison_panel',
                                          ...
                                          wellPanel(id = 'row06',
                                                    ...
                                                    fluidRow(
                                                      ...
                                                      column(4,
                                                             div(HTML({paste('<p style = "font-size:1.5rem; color:black; font-weight:bold; text-align: center;margin-left: -100px; margin-right: -100px">Middle Eastern Restaurants', '</p>')}))
                                                      ),
                                                      column(4,
...
server = function(input, output, session){
  ...
  ##########################
  # output$match_result <- renderText({''})
  # output$customer_match <- renderText({''})
  # output$income_match <- renderText({''})
  # output$school_match<- renderText({''})
  # output$childcare_match <- renderText({''})
  # output$legal_match <- renderText({''})
  ...
  observeEvent(input$search, {
    ...
    # # childcare
    childcare_output = HTML('<p style="color:black; font-size: 15px; text-align: center;">',length(subset(childcare, Suburb == selected_suburb)[,1]),'</p>')
    output$childcare_match <- renderText({childcare_output})
    # # legal
    legal_output = HTML('<p style="color:black; font-size: 15px; text-align: center;">',length(subset(legal, Suburb == selected_suburb)[,1]),'</p>')
  ...
  observeEvent(input$map_shape_click, {
    ...
    if (!is.null(selected_suburb)){
      ...
      #shinyjs::show('highlevel4') # show zoomed in restaurant info
  ...
  observeEvent(input$map_shape_click, { 
    ...
    if (!is.null(selected_suburb)){
      ...
      ### if the suburb has legal services AND schools
