...
cuisine_to_search <- cuisine_reference[["MDE"]]
...
server = function(input, output, session){
  ...
  observeEvent(input$map_shape_click, { 
    ...
    if (!is.null(selected_suburb)){
      ...
      output$map <- renderLeaflet({
        ...
        hideGroup(group = control_group[5:6]) %>%
          ...
          addPolygons(data = vic,
                      weight = 0,
                      stroke = 0,
                      fillColor = ~mypal_me(vic$ME), # heatmap colour 
                      fillOpacity = 0,
                      label = ~suburb,
                      labelOptions = labelOptions(
                        opacity = 0),
      ...
      data <- search(cityID, api_key = key1, query = selected_suburb, cuisine = cuisine_to_search)
      ...
      if (!typeof(data) == 'logical'){
        ...
        if (length(data[[1]] >0)){
          ...
          existing_business = HTML({paste('<p style="color:#3A479B;  font-size: 15px;">',paste('Number of Middle Eastern Resturants: <strong>',no),'</strong></p>')})
          ...
          existing_business = HTML('<p style="color:#3A479B; font-size: 15px;">Number of Middle Eastern Restaurants: <strong>0</strong></p>')
          output$existing_business <- renderText({ 
            existing_business
          })
          output$restaurant_text <- renderText({
