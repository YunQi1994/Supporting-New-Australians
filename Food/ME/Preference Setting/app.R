...
dummy <- read.csv('data/ranking_me.csv', stringsAsFactors = F)
...
server <- function(input, output, session){
  ...
  observe({
    ...
    output$business01 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Mid-Eatern Restaurants: ',get_business_count(suburb01),'</p>')})
    })
    ...
    output$business02 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Mid-Eatern Restaurants: ',get_business_count(suburb02),'</p>')})
    })
    ...
    output$business03 <- renderText({
      HTML({paste('<p style="color:#16224D;  font-weight:bold; font-size: 12px"> Mid-Eatern Restaurants: ',get_business_count(suburb03),'</p>')})
    })
